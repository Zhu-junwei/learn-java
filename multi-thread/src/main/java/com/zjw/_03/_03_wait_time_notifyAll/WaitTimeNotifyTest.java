...
class WaitThread extends Thread {
    ...
    public void run() {
        synchronized (object) {
            try {
                System.out.println(DateTime.now().toString("yyyy-MM-dd HH:mm:ss.SSS") + " wait thread...start..." + Thread.currentThread().getName() + "...");
                //使当前线程等待10S，10S后如果有锁就继续往下执行
                //如果不能获得锁，则会继续等待
                object.wait(10000);
                System.out.println(DateTime.now().toString("yyyy-MM-dd HH:mm:ss.SSS") + " wait thread...end..." + Thread.currentThread().getName() + "...");
